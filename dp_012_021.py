...
# 以下、局所距離の計算
    ...
    for a in range(0, 100):  # a = 未知単語の数
        ...
        for x in range(0, 100):  # x = テンプレ単語の数
            ...
            for b in range(0, sp012_shape[x][0]):  # テンプレート、フレーム数Iに相当 (ex.city011_001 : 61)

                for c in range(0, sp021_shape[a][0]):  # 未知、フレーム数Jに相当 (ex.city012_001 : 64)

                    for d in range(0, 15):  # 次元数１５
                        dis_part = Speaker_01_2[x][b][d] - Speaker_02_1[a][c][d]  # 話者どうしの引き算
...
# 以下、累積距離の計算
    ...
    # 以下、境界条件の設定
            for a in range(1, sp021_shape[x][0]):  # 列（未知）の境界条件
            ...
            i = 1
            for a in range(1, sp012_shape[y][0]):  # 行（テンプレート）の境界条件
    ...
    for x in range(0, 100):  # Unknown
        ...
        for y in range(0, 100):  # Template
            xy_2 = (x*100)+y
            for i in range(1, sp012_shape[y][0]):

                for j in range(1, sp021_shape[x][0]):
            ...
            T = DP_array_all[xy_2][sp012_shape[y][0]-1][sp021_shape[x][0]-1] / (sp012_shape[y][0] + sp021_shape[x][0])
    ...
    print(check_list)
    print("012_021, 最小累積距離[最大値：最小値：平均]：[" + str(max(copy(Mdistance_list_min))) + "：" + str(min(copy(Mdistance_list_min))) + "：" + str(copy(sum(Mdistance_list_min)/len(Mdistance_list_min))) + "]")
    print("012_021, 一致率：" + str(check_list.count(1)) + " %")
