            ...
            for b in range(0, sp011_shape[x][0]):  # テンプレート、フレーム数Iに相当 (ex.city011_001 : 61)
                ...
                for c in range(0, sp021_shape[a][0]):  # 未知、フレーム数Jに相当 (ex.city012_001 : 64)
                    ...
                    for d in range(0, 15):  # 次元数１５
                        dis_part = Speaker_01_1[x][b][d] - Speaker_02_1[a][c][d]  # 話者どうしの引き算
...
# 以下、累積距離の計算
    ...
    for x in range(0, 100):  # Unknown
        ...
        for y in range(0, 100):  # Template
            ...
            T = DP_array_all[xy_2][sp011_shape[y][0]-1][sp021_shape[x][0]-1] / (sp011_shape[y][0] + sp021_shape[x][0])
    ...
    print(check_list)
    print("011_021, 最小累積距離[最大値：最小値：平均]：[" + str(max(copy(Mdistance_list_min))) + "：" + str(min(copy(Mdistance_list_min))) + "：" + str(copy(sum(Mdistance_list_min)/len(Mdistance_list_min))) + "]")
    print("011_021, 一致率：" + str(check_list.count(1)) + " %")
